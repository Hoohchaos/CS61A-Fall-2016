...
def tree_map(fn, t):
    ...
    """
    "*** YOUR CODE HERE ***"
    return Tree(fn(t.root),[tree_map(fn, i) for i in t.branches])
...
def add_trees(t1, t2):
    ...
    """
    "*** YOUR CODE HERE ***"
    pairs=list(zip(t1.branches,t2.branches))
    if len(t1.branches)>len(t2.branches):
    	pairs+=[[tr,Tree(0)] for tr in t1.branches[len(t2.branches):]]
    elif len(t2.branches)>len(t1.branches):
    	pairs+=[[tr,Tree(0)] for tr in t2.branches[len(t1.branches):]]
    return Tree(t1.root+t2.root, [add_trees(pair[0],pair[1]) for pair in pairs])
