    ...
    def remove_insect(self, insect):
       ...
       """
        if insect.is_ant:
            ...
            if hasattr(insect, 'true_queen') and insect.true_queen: # can't remove the true queen
                return None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
       ...
       """
        ...
        while curr_place != hive: # nearest bee that is not in the HIVE
            bee_list = curr_place.bees # get list of bees for current place
            if len(bee_list) > 0 : # some bees found
                return random_or_none(bee_list)
            else: # check next place, which is the current place's entrance
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        # skip the first min_range places ahead
        for i in range(0, self.min_range):
            curr_place = curr_place.entrance
 
        # same as ThrowerAnt's nearest_bee
        while curr_place != hive:
            bee_list = curr_place.bees
            if len(bee_list) > 0 :
                return random_or_none(bee_list)
            else:
                curr_place = curr_place.entrance
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        # only search current place and max_range places ahead
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6B
        if (bee is not None) and (self.digesting <= 0): # eat the bee and set digesting
            self.digesting = self.time_to_digest
            bee.reduce_armor(bee.armor)
        else: # decrement digesting
            self.digesting -= 1
        # END Problem 6B
 
    def action(self, colony):
        # BEGIN Problem 6B
        return self.eat_bee(random_or_none(self.place.bees)) # pick a random bee in the current place to eat
        # END Problem 6B
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    food_cost = 7
    num_queens = 0
    true_queen = False # we'll check if it's true (first queen) on init
    implemented = False # Change to True to view in the GUI
    ...
    def action(self, colony):
       ...
       """
        ...
        # kill off fake queens and stop action
        ...
        while place:
            ...
            if ant:
                ...
                # is it containing an inside_ant?
                if isinstance(ant, BodyguardAnt):
                    inside_ant = ant.ant
                else:
                    inside_ant = None
 
                if ant not in self.doubled: # ant not in doubled
                    ant.damage *= 2
                    self.doubled.append(ant)
                if inside_ant and inside_ant not in self.doubled: # inside_ant exists and not in doubled
                    inside_ant.damage *= 2
                    self.doubled.append(inside_ant)
            place = place.exit
    ...
    def reduce_armor(self, amount):
       ...
       """
        # BEGIN Problem 9
        self.armor -= amount
        if self.true_queen: # true queen has no armor, bees win :(
            bees_win()
        else:
            self.place.remove_insect(self) # remove this imposter
        # END Problem 9
